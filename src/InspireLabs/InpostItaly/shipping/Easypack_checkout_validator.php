...
class Easypack_checkout_validator
{
            ...
            foreach ($fieldset as $key => $field) {
                ...
                switch ($fieldset_key) {
                    case 'shipping' :
                        ...
                        $field_label = sprintf(__('Shipping %s', 'inpost-italy'), $field_label);
                        break;
                    case 'billing' :
                        /* translators: %s: field name */
                        $field_label = sprintf(__('Billing %s', 'inpost-italy'), $field_label);
                        break;
                }

                if (in_array('phone', $format)) {

                    if (false === $this->is_it_phone($data[$key])) {

                    $errors->add('validation', sprintf(__('Please enter a valid IT phone number.', 'inpost-italy'),
     ...
     *
     */
    private function is_it_phone($phone)
    {
        //$regex = "/^([0-9]{10,11})$/";
        $regex = "/^([0-9]{10,11})$/";
