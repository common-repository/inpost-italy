...
namespace InspireLabs\InpostItaly\shipx\services\organization;

use Exception;
use InspireLabs\InpostItaly\shipx\models\organization\services\ShipX_Additional_Service_Model;
use InspireLabs\InpostItaly\shipx\models\organization\services\ShipX_Service_Model;
use InspireLabs\InpostItaly\shipx\models\organization\ShipX_Organization_Model;
...
class ShipX_Organization_Service {
	...
	public function query_organisation( $id = null ) {
        ...
        for ( $x = 0; $x <= 3; $x ++ ) {
            ...
            $organization = inpost_italy_api()->get_organization( $id );
            if ( ! empty( $organization ) ) {
                update_option( 'inpost_italy_organisation', $organization );
                break;
            }
            sleep(1);
        }
        ...
        for ( $x = 0; $x <= 3; $x ++ ) {
            ...
            $allServices = inpost_italy_api()->getServicesGlobal();

            if ( ! empty( $allServices ) ) {
                update_option( 'inpost_italy_organisation_global', $allServices );
                break;
            }
            sleep(1);
        }
		...
		foreach ( $allServices as $k => $service ) {
			if ( in_array( $service['id'], $services_for_organisation ) ) {
				...
				$additional_services = [];
				foreach (
					$service['additional_services'] as $additional_service
				) {
