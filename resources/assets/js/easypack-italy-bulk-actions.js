...
jQuery(document).ready(function(){
    ...
    jQuery('#doaction').on('click',function(e) {
        ...
        /*if(action === 'easypack_bulk_create_shipments_then_labels') {
            e.preventDefault();
            let selected_data = inpost_italy_table_processing();
            if( typeof selected_data != 'undefined' && selected_data !== null ) {
                if( Object.keys(selected_data.orders).length ) {
                    inpost_italy_process_selected_item( selected_data.orders, 1, selected_data.selected_row_count, form, 0, 1 );
                }
            }
        }*/

        // only shipments
        if( action === 'easypack_italy_bulk_create_shipments' ) {
            e.preventDefault();
            let selected_data = inpost_italy_table_processing();

            if( typeof selected_data != 'undefined' && selected_data !== null ) {
                if( Object.keys(selected_data.orders).length ) {
                    inpost_italy_process_selected_item( selected_data.orders, 1, selected_data.selected_row_count, form, 0, 0 );
                }
            }
        }

        // only labels
        if(action === 'easypack_italy_bulk_create_labels') {
            e.preventDefault();
            ...
            if( typeof selected_data != 'undefined' && selected_data !== null ) {
                if (Object.keys(selected_data.orders).length) {
                    inpost_italy_print_labels_bulk(selected_data.orders);
                }
            }
        }
    });
...
document.addEventListener('click', function (e) {
    ...
    if ( target.classList.contains('dashicons-media-spreadsheet') ) {
        ...
        inpost_italy_print_labels_bulk([order_id]);


    }
});

function inpost_italy_print_labels_bulk(orders) {
    ...
    var easy_action = 'easypack_italy_create_bulk_labels';
    var order_ids = JSON.stringify(orders);
    var security = easypack_it_bulk.easypack_nonce;
		
    beforeSend();
    var request = new XMLHttpRequest();
    request.open('POST', easypack_it_bulk.ajaxurl, true);
    ...
    request.onload = function() {
        ...
        if( request.status === 200 && request.response.size > 0 ) {
            ...
            if( content_type === 'application/zip' ) {
                ...
                filename = 'inpost_ordini' + file_name_part + '.zip';
                blob = new Blob([request.response], { type: 'application/zip' });

            } else if( content_type === 'application/pdf' ) {
                filename = 'inpost_ordine' + file_name_part + '.pdf';
                ...
                reader.onload = function() {
                    ...
                    if( textResponse.details.key == 'ParcelLabelExpired' ) {
                        alert('Probabilmente 1 etichetta è scaduta');
                    } else {
                        alert("Si è verificato un errore durante il download delle etichette");
    ...
    request.send('action=' + general_action + '&easypack_action=' + easy_action +'&security=' + security + '&order_ids=' + order_ids);

}


function inpost_italy_process_selected_item(orders, index, total, form, failed, need_labels = false) {
    ...
    ajaxdata_process_item['action'] = 'easypack_italy_bulk_create_shipments';
    ajaxdata_process_item['security'] = easypack_it_bulk.easypack_nonce;
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data.status === 'ok') {
                ...
                if (index <= total) {
                    inpost_italy_process_selected_item(orders, (index + 1), total, form, failed, need_labels);
                }
                ...
                if( typeof data.message != 'undefined' && data.message !== null ) {
                    ...
                    if( cleared_message_text.indexOf('height required') !== -1
                        ...
                        cleared_message_text = 'Controllare che le dimensioni della confezione siano corrette';
            ...
            if (index == total) {
                if( need_labels ) {
                    inpost_italy_print_labels_bulk( successful_ids );
                }
            }
...
function inpost_italy_table_processing() {
    var form = jQuery('#posts-filter'),
    ...
    if (selected_row_count === 0) {
        alert('Non sono stati selezionati ordini');
        return;
    } else {
